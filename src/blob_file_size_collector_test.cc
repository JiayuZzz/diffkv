...
TEST_F(BlobFileSizeCollectorTest, Basic) {
  ...
  std::map<uint64_t, BlobFileData> result;
  BlobFileSizeCollector::Decode(&raw_blob_file_size_prop, &result);

  ASSERT_EQ(2, result.size());

  ASSERT_EQ(kNumEntries / 2 * 10, result[0].blob_files_size_);
  ASSERT_EQ(kNumEntries / 2 * 10, result[1].blob_files_size_);
  ASSERT_EQ(kNumEntries / 2, result[0].blob_files_entries_);
  ASSERT_EQ(kNumEntries / 2, result[1].blob_files_entries_);
}
