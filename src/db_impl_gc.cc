...
Status TitanDBImpl::BackgroundGC(LogBuffer* log_buffer,
  ...
  Status s;
  {
    ...
    if (UNLIKELY(!blob_gc)) {
      ...
      auto cf_id = blob_gc->column_family_handle()->GetID();
      auto cf_options = blob_file_set_->GetBlobStorage(cf_id).lock()->cf_options();

      bg_gc = ((total_size>live_size && (double)(total_size-live_size)/total_size > cf_options.blob_file_discardable_ratio) || block_for_size_.load()) && (bg_gc_scheduled_ - 1 + gc_queue_.size() <
